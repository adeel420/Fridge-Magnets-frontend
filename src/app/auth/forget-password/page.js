...
const Page = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!email) {
      return setShowErrorMsg("Please enter your email address.");
    }
    setLoading(true);
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        setShowMsg(response.data.msg || "You can verify the email");
        setShowErrorMsg("");
        setEmail("");
      } else {
        setShowErrorMsg(response.data.msg);
        setShowMsg("");
      }
    } catch (err) {
      setShowErrorMsg(err);
    ...
    <div className="bg-[#f9fafc] min-h-screen flex items-center justify-center pt-42 pb-8 px-4">
      ...
      <div className="bg-white w-full max-w-md p-8 rounded-lg shadow-lg">
        ...
        <p className="text-[#677077] text-center mt-2 text-sm">
          Forget password to access your{" "}
        ...
        <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="w-full cursor-pointer bg-[#dd492b] hover:bg-[#b93c1f] text-white py-2 rounded font-semibold transition"
          >
            Confirm
          </button>
